...
function normalizeAppointmentInput(
  ...
  return {
    ...
    teleconsultationPlatform: getTeleconsultationPlatformLabel(
      input.teleconsultationPlatform,
    ),
    teleconsultationUrl: null,
...
function mapAppointmentRow(
  ...
  return {
    ...
    teleconsultationAccessInstructions:
      row.teleconsultation_access_instructions ?? null,
...
function getDemoPatientByIdentity(
  ...
  if (userId) {
    const byUserId = database.patients.find(
      (patient: Patient) => patient.userId === userId,
    );
    if (byUserId) {
      return byUserId;
    }
  ...
  return (
    database.patients.find(
      (patient: Patient) => patient.email.toLowerCase() === email.toLowerCase(),
    ) ?? null
  );
}
...
export async function getTeleconsultAppointmentsForUser(input: {
  ...
  if (!isSupabaseConfigured) {
    ...
    const patient =
      role === "patient"
        ? getDemoPatientByIdentity(input.userId, input.email)
        : null;
    ...
    return appointments.map((appointment) => {
      const linkedPatient = database.patients.find(
        (item: Patient) => item.id === appointment.patientId,
      );
      const linkedDoctor = database.users.find(
        (item: UserProfile) => item.id === appointment.doctorId,
      );
      const linkedService = database.services.find(
  ...
  if (role === "patient") {
    const { data, error: patientError } = await client
      .from("patients")
      .select("id")
  ...
  const appointments = ((data ?? []) as AppointmentRowLoose[]).map(
    mapAppointmentRow,
  );
  ...
  const serviceIds = [
    ...new Set(
      appointments.map((appointment) => appointment.serviceId).filter(Boolean),
    ),
  ];
